...
// ---------------------------------------------------------------------------
// Mod Abuse Detection settings page (Premium)
// ---------------------------------------------------------------------------

export async function handleModAbuseSettings(
  ...
  const page = await buildModAbusePage(guildId);
  await interaction.update(page);
}

async function buildModAbusePage(guildId: string): Promise<{
  embeds: ReturnType<typeof buildSettingsEmbed>[];
  components: ActionRowBuilder<ButtonBuilder>[];
}> {
  const settings = await getGuildSettings(guildId);
  const cfg = settings.modAbuse;
  ...
  const [
    ...
    t(guildId, 'settings.modAbuse.title'),
  ...
  const embed = buildSettingsEmbed(title, [
    ...
    { name: fieldThreshold, value: `${cfg.threshold} actions`, inline: true },
    { name: fieldWindow, value: `${cfg.windowSeconds}s`, inline: true },
  ]);

  const toggleBtn = new ButtonBuilder()
    .setCustomId(makeButtonId('settings', 'modAbuse', 'toggle'))
  ...
  const thresholdBtn = new ButtonBuilder()
    .setCustomId(makeButtonId('settings', 'modAbuse', 'threshold'))
    .setLabel(btnSetThreshold)
    .setStyle(ButtonStyle.Secondary);

  const windowBtn = new ButtonBuilder()
    .setCustomId(makeButtonId('settings', 'modAbuse', 'window'))
...
export async function handleModAbuseToggle(interaction: ButtonInteraction): Promise<void> {
  if (!interaction.inGuild()) return;
  const guildId = interaction.guildId;

  await updateGuildSettings(guildId, (s) => ({
    ...s,
    modAbuse: { ...s.modAbuse, enabled: !s.modAbuse.enabled },
  }));
...
export async function showModAbuseThresholdModal(interaction: ButtonInteraction): Promise<void> {
  if (!interaction.inGuild()) return;
  const guildId = interaction.guildId;
  const settings = await getGuildSettings(guildId);
  ...
  const modal = new ModalBuilder()
    .setCustomId(makeButtonId('settings', 'modAbuse', 'threshold_submit'))
    .setTitle(title)
    .addComponents(
      new ActionRowBuilder<TextInputBuilder>().addComponents(
        new TextInputBuilder()
          ...
          .setValue(String(settings.modAbuse.threshold))
          .setMinLength(1)
          .setMaxLength(4)
          .setRequired(true),
      ),
...
export async function handleModAbuseThresholdSubmit(
  ...
  await updateGuildSettings(guildId, (s) => ({
    ...s,
    modAbuse: { ...s.modAbuse, threshold: val },
  }));
...
export async function showModAbuseWindowModal(interaction: ButtonInteraction): Promise<void> {
  if (!interaction.inGuild()) return;
  const guildId = interaction.guildId;
  const settings = await getGuildSettings(guildId);
  ...
  const modal = new ModalBuilder()
    .setCustomId(makeButtonId('settings', 'modAbuse', 'window_submit'))
    .setTitle(title)
    .addComponents(
      new ActionRowBuilder<TextInputBuilder>().addComponents(
        new TextInputBuilder()
          ...
          .setLabel(label)
          .setStyle(TextInputStyle.Short)
          .setValue(String(settings.modAbuse.windowSeconds))
          .setMinLength(1)
          .setMaxLength(4)
          .setRequired(true),
      ),
...
export async function handleModAbuseWindowSubmit(
  interaction: ModalSubmitInteraction,
): Promise<void> {
  if (!interaction.inGuild()) return;
  const guildId = interaction.guildId;
  ...
  await updateGuildSettings(guildId, (s) => ({
    ...s,
    modAbuse: { ...s.modAbuse, windowSeconds: val },
  }));
